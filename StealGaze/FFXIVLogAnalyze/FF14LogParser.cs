...
namespace FFXIV_Tools
{
    ...
    public class FF14LogParser
    {
        ...
        public FFXIVLogDataSet.AnaylzedRow Add(FFXIVLog log)
        {
            ...
            {//Toに名前が入っていれば
                ...
                if(need)// FFXIVLog.GetBATTLE_TYPE(arow.ac (arow.ActionType == 0x2B || arow.ActionType == 0xAB))
                {
                    ...
                    {//ME
                        acrow_to.IsMe = ptype == 0;
                        acrow_to.IsMyPet = ptype == 1;
                        acrow_to.IsNPC = !acrow_to.IsMe && !acrow_to.IsMyPet;
                    }
                    else if(arow.FLAG_TO == (int)LOG_CATEGORY_TO.PTMEMBER)
                    {//PT
                        acrow_to.IsPTMember = ptype == 0;
                        acrow_to.IsPTPet = ptype == 1;
                        acrow_to.IsNPC = !acrow_to.IsPTMember && !acrow_to.IsPTPet;
                    }
                    else if(arow.FLAG_TO == (int)LOG_CATEGORY_TO.ENEMY)
                    {
                        acrow_to.IsOther = ptype == 0 || ptype == 1;
                        acrow_to.IsNPC = !acrow_to.IsOther;
                    }
            ...
            {//Fromに名前が入っていれば
                ...
                if(!arow.IsNumericNull() && arow.ActionName != "攻撃")
                {
                    ...
                    if(action != null && action.DD威力 > 0)
                    {
                        ...
                        {
                            double ratio = arow.IsCritical ? 1.5 : 1.0;
                            ratio = arow.BonusRate > 0 ? (100 + arow.BonusRate) / 100.0 : ratio;
                            acrow_from.DamageBase = (double)arow.Numeric / ratio / action.DD威力;
                        }
                    }
                }
                //Actorを更新する
                if(true)//(need)// FFXIVLog.GetBATTLE_TYPE(arow.ac (arow.ActionType == 0x2B || arow.ActionType == 0xAB))
                {
                    ...
                    {//ME
                        acrow_from.IsMe = ptype == 0;
                        acrow_from.IsMyPet = ptype == 1;
                        acrow_from.IsNPC = !acrow_from.IsMe && !acrow_from.IsMyPet;
                    }
                    else if(arow.LogType >= 0x08 && arow.LogType <= 0x0B)
                    {//PT
                        acrow_from.IsPTMember = ptype == 0;
                        acrow_from.IsPTPet = ptype == 1;
                        acrow_from.IsNPC = !acrow_from.IsPTMember && !acrow_from.IsPTPet;
                    }
                    else if(arow.LogType >= 0x0C && arow.LogType <= 0x0F)
                    {//Alliance
                        acrow_from.IsAllianceMember = ptype == 0 || ptype == 1;
                    }
                    else if(arow.LogType >= 0x10 && arow.LogType <= 0x13)
                    {
                        acrow_from.IsOther = ptype == 0 || ptype == 1;
                    }
                    else if(arow.LogType >= 0x14)
                    {
                        acrow_from.IsOther = ptype == 0 || ptype == 1;
                        acrow_from.IsNPC = !acrow_from.IsOther;
                    }
            ...
            if(((acrow_from != null && acrow_from.Name != FF14LogParser.UnknownName) && (acrow_to != null && acrow_to.Name != FF14LogParser.UnknownName)) && (acrow_from.IsMe || acrow_from.IsMyPet || acrow_from.IsPTMember || acrow_from.IsPTPet) && !arow.IsActionNameNull())
            {
        ...
        private int GetPlayerTypeFromName(string name)
        {
            ...
            else if(FF14LogParser.IsPet(name))
            {
                return 1;
            }
            else
        ...
        private void SetDotSpace(FFXIVLogDataSet.AnaylzedRow arow)
        {
            ...
            arow.To.Replace("'", "''"), arow.EnemyGroupID, sames[0].ID), "ID");
        ...
        /// <param name="arow"></param>
        /// <param name="log"></param>
        private void PvPStartEnd(FFXIVLogDataSet.AnaylzedRow arow, FFXIVLog log)
        {
            arow.ContentName = "ウルヴズジェイル";
            string _logstring = log.LogBodyReplaceTabCode;
            if(_logstring == ":バトル開始までお待ちください。")
            {
                arow.IsPvPStart = true;
                ds.ContentRegion.AddContentRegionRow(arow.ID, -1, arow.ContentName, true, false);
            }
            if(_logstring.EndsWith("PvP EXPを得た。"))
            {
                arow.IsPvPEnd = true;
                FFXIVLogDataSet.ContentRegionRow[]rows= (FFXIVLogDataSet.ContentRegionRow[])ds.ContentRegion.Select(String.Format("ContentName = '{0}'", arow.ContentName.Replace("'", "''")));
                if(rows.Length > 0)
        ...
        private void CureAnalyze(FFXIVLogDataSet.AnaylzedRow arow, FFXIVLog log)
        {
            ...
            arow.IsHP = match.Groups["hpmp"].Value == "ＨＰ";
            arow.IsTP = match.Groups["hpmp"].Value == "ＴＰ";
            arow.IsMP = match.Groups["hpmp"].Value == "ＭＰ";
        ...
        private void EffectAnalyze(FFXIVLogDataSet.AnaylzedRow arow, FFXIVLog log)
        {
            ...
            arow.IsBuffOn = match.Groups["on"].Value != "";
            arow.IsDebuffRemove = match.Groups["off"].Value != "";
        ...
        private void HitAnalyze(FFXIVLogDataSet.AnaylzedRow arow, FFXIVLog log)
        {
            ...
            else
            {
                ...
                arow.IsBlock = match.Groups["dodge"].Value.Contains("ブロック");
                arow.IsParry = match.Groups["dodge"].Value.Contains("受け流し");
            }
            if(arow.From == "")
            {
                SearchAction(arow, log);
        ...
        {
            string convertFrom = baseName;
            if(FF14LogParser.IsPet(baseName))
            {
                switch(log.WHO)
                {
                    case LOG_CATEGORY_WHO.MYSELF:
                        convertFrom += "(MY)";
                        break;
                    case LOG_CATEGORY_WHO.PTMEMBER:
                        convertFrom += "(PT)";
                        break;
                    case LOG_CATEGORY_WHO.ENEMY:
                        convertFrom += "(EN)";
                        break;
                    default:
                        convertFrom += "(OTHER)";
        ...
        {
            string convertTo = baseName;
            if(FF14LogParser.IsPet(baseName))
            {
                switch(log.TO)
                {
                    case LOG_CATEGORY_TO.EMPTY_OR_ME:
                        convertTo += "(MY)";
                        break;
                    case LOG_CATEGORY_TO.PTMEMBER:
                        convertTo += "(PT)";
                        break;
                    case LOG_CATEGORY_TO.ENEMY:
                        convertTo += "(EN)";
                        break;
                    default:
                        convertTo += "(OTHER)";
        ...
        public static bool IsPet(string name)
        {
            var replacedName = Regex.Replace(name, @"\(.+?\)", "");
            string[] petNames = { "カーバンクル・エメラルド", "カーバンクル・トパーズ", "イフリート・エギ", "タイタン・エギ", "ガルーダ・エギ", "フェアリー・エオス", "フェアリー・セレネ" };
            return petNames.Contains(replacedName);
        }
        ...
        private void SetAction()
        {
            ...
            ds.Action.AddActionRow("サンダー", "呪術士 黒魔道士", 30, 35, 18, true);
            ...
            ds.Action.AddActionRow("ブリザラ", "幻術士 呪術士 巴術士 白魔道士 黒魔道士 召喚士 学者", 100, 0, 0, true);
            ...
            // PvPアクション
            // TODO: 威力等の設定
            ds.Action.AddActionRow("浄化", "剣術士 ナイト 斧術士 戦士 格闘士 モンク 槍術士 竜騎士 弓術士 吟遊詩人 幻術士 呪術士 巴術士 白魔道士 黒魔道士 召喚士 学者", 0, 0, 0, false);
            ds.Action.AddActionRow("奮起", "剣術士 ナイト 斧術士 戦士 格闘士 モンク 槍術士 竜騎士 弓術士 吟遊詩人", 0, 0, 0, false);
            ds.Action.AddActionRow("フルスイング", "剣術士 ナイト 斧術士 戦士", 0, 0, 0, true);
            ds.Action.AddActionRow("テストゥド", "剣術士 ナイト", 0, 0, 0, false);
            ds.Action.AddActionRow("グローリースラッシュ", "剣術士 ナイト", 0, 0, 0, true);
            ds.Action.AddActionRow("ミスリルテンペスト", "斧術士 戦士", 0, 0, 0, true);
            ds.Action.AddActionRow("スリル・オブ・ウォー", "斧術士 戦士", 0, 0, 0, false);
            ds.Action.AddActionRow("フェターウォード", "格闘士 モンク 槍術士 竜騎士", 0, 0, 0, false);
            ds.Action.AddActionRow("ウェポンスロー", "格闘士 モンク 槍術士 竜騎士", 0, 0, 0, true);
            ds.Action.AddActionRow("インパルスラッシュ", "槍術士 竜騎士", 0, 0, 0, true);
            ds.Action.AddActionRow("スキュアー", "槍術士 竜騎士", 0, 0, 0, true);
            ds.Action.AddActionRow("かかと落とし", "格闘士 モンク", 0, 0, 0, true);
            ds.Action.AddActionRow("サマーソルト", "格闘士 モンク", 0, 0, 0, true);
            ds.Action.AddActionRow("マナドロー", "弓術士 吟遊詩人 幻術士 呪術士 巴術士 白魔道士 黒魔道士 召喚士 学者", 0, 0, 0, false);
            ds.Action.AddActionRow("マナソング", "弓術士 吟遊詩人", 0, 0, 0, false);
            ds.Action.AddActionRow("ファーショット", "弓術士 吟遊詩人", 0, 0, 0, true);
            ds.Action.AddActionRow("ブラストショット", "弓術士 吟遊詩人", 0, 0, 0, true);
            ds.Action.AddActionRow("専心", "幻術士 呪術士 巴術士 白魔道士 黒魔道士 召喚士 学者", 0, 0, 0, false);
            ds.Action.AddActionRow("エーテルバースト", "幻術士 呪術士 巴術士 白魔道士 黒魔道士 召喚士 学者", 0, 0, 0, true);
            ds.Action.AddActionRow("ナイトウィング", "呪術士 黒魔道士", 0, 0, 0, false);
            ds.Action.AddActionRow("ファントムダート", "呪術士 黒魔道士", 0, 0, 0, true);
            ds.Action.AddActionRow("ミスティヴェール", "巴術士 召喚士", 0, 0, 0, false);
            ds.Action.AddActionRow("ウィザー", "巴術士 召喚士", 0, 0, 0, true);
            ds.Action.AddActionRow("女神の息吹", "幻術士 白魔道士 学者", 0, 0, 0, false);
            ds.Action.AddActionRow("調和", "幻術士 白魔道士 学者", 0, 0, 0, false);
            ds.Action.AddActionRow("女神の慈悲", "幻術士 白魔道士", 0, 0, 0, false);
            ds.Action.AddActionRow("生命波動法", "学者", 0, 0, 0, true);
            ds.Action.AddActionRow("精神統一の策", "学者", 0, 0, 0, false);
